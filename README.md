...
```bash
docker build -t speakit-backend .
...
Create `src/public/runtime-env.js` for local development:

```javascript
// src/public/runtime-env.js  (local dev — do not commit)
window.__env = {
  API_URL: "http://localhost:8080",
};
```

For production (Vercel), this file is generated by the `prebuild` npm hook pointing to your Render deployment URL.
...
Contributions are welcome. All changes must go through a feature branch — direct commits to `main` are not accepted.
