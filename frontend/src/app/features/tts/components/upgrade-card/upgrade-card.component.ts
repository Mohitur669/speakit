...
@Component({
  ...
  template: `
    <div
      class="hidden md:block bg-white dark:bg-primary-900 p-4 rounded-xl border border-accent-500/20 transition-all group hover:border-accent-500/50"
    >
      <div class="flex items-center gap-2.5 mb-3">
        <div
          class="w-7 h-7 rounded-lg bg-accent-500/10 flex items-center justify-center text-accent-500 shrink-0"
        >
        ...
        <h3 class="text-[10px] font-bold text-primary-900 dark:text-white tracking-widest">
          {{ title }}
        </h3>
      </div>

      <p class="text-[9px] leading-relaxed text-primary-500 dark:text-primary-400 mb-4">
        {{ description }}
      </p>

      <button
        (click)="upgrade.emit(nextPlan)"
