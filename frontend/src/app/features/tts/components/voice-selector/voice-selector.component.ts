...
@Component({
  ...
  template: `
    <div
      class="flex-1 bg-white dark:bg-primary-900 rounded-xl border border-primary-300 dark:border-primary-700 p-4 sm:p-6 flex flex-col gap-4"
    >
      <div class="flex flex-wrap items-center justify-start gap-3 mb-2 sm:mb-4">
        <h2
          class="text-sm font-bold text-primary-900 dark:text-white tracking-widest whitespace-nowrap"
        >
          Settings
        </h2>
        <!-- Dynamic Voice Count Indicator -->
        <div
          class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-brand-blue/5 dark:bg-brand-blue/10 border border-brand-blue/10"
        >
      ...
      <div class="grid grid-cols-2 gap-3 sm:gap-4">
        ...
        <div class="relative">
          ...
          >
          <button
            (click)="toggleEngineDropdown($event)"
          ...
          >
            ...
            <svg
              ...
              [class.rotate-180]="isEngineDropdownOpen"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      @if (currentFilter() === 'Indian') {
        <div class="relative animate-fade-in">
          <label class="block text-xs font-semibold text-primary-500 mb-2 tracking-widest px-1"
            >Regional Language</label
          >
          <button
              ...
              @for (lang of languageOptions(); track lang) {
                ...
                >
                  <div
                    ...
                    "
                  >
                    @if (selectedLanguages().has(lang.code)) {
                      <svg
                        class="w-3 h-3 text-white"
                        fill="none"
                        stroke="currentColor"
                  ...
                  <span
                    class="text-xs font-bold text-primary-700 dark:text-primary-200 group-hover:text-primary-900 dark:group-hover:text-white truncate"
                    >{{ lang.name }}</span
                  >
                </button>
              }
      ...
      <div class="relative">
        ...
        >
          <div class="flex items-center gap-3">
            <div
              class="w-11 h-11 rounded-xl bg-white dark:bg-primary-900 border border-primary-300 dark:border-primary-700 flex items-center justify-center text-2xl shadow-sm group-hover:scale-105 transition-transform"
            >
              {{
          ...
          <svg
            ...
            [class.rotate-180]="isVoiceDropdownOpen"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        @if (isVoiceDropdownOpen) {
          <div
            (click)="$event.stopPropagation()"
            class="absolute top-full mt-2 left-0 right-0 bg-white dark:bg-primary-900 border border-primary-300 dark:border-primary-700 rounded-xl shadow-2xl z-[60] overflow-hidden animate-fade-in"
          >
            <div class="max-h-72 overflow-y-auto custom-scrollbar">
              @for (voice of getFilteredVoices(); track voice) {
                ...
                >
                  <div class="flex items-center gap-3">
                    <span class="text-lg group-hover/item:scale-110 transition-transform">{{
                      voice.gender === 'Female' ? '👩' : '👨'
                    }}</span>
                    <div>
                      <div
                        class="text-sm font-bold text-primary-900 dark:text-white"
                        [class.text-brand-blue]="voiceId === voice.id"
                      >
                        {{ voice.name }}
                      </div>
                      <div class="text-[10px] text-primary-400 font-bold tracking-wide">
                        {{ voice.gender }} · {{ voice.languageCode }}
                      </div>
                    </div>
                  </div>
                  <span
                    [ngClass]="getVoiceTypeClass(voice)"
                    class="text-[9px] font-extrabold tracking-widest px-2 py-0.5 rounded bg-primary-100 dark:bg-primary-800"
                  >
              ...
              @if (getFilteredVoices().length === 0) {
                <div class="p-8 text-center">
                  <p class="text-xs text-primary-500 font-medium italic">
...
export class VoiceSelectorComponent {
  ...
  setFilter(filter: string): void {
    ...
    if (filter === 'Indian' && !this.userCanUseSarvam) {
      this.showNotification.emit({
        message: 'Indian AI voices require a PRO subscription',
        type: 'error',
      });
      return;
    }
