...
@Component({
  ...
  template: `
    <div
      class="flex flex-col items-center justify-center p-12 text-center bg-white dark:bg-primary-900 rounded-3xl border border-primary-300 dark:border-primary-700 shadow-xl max-w-2xl mx-auto animate-fade-in"
    >
      <div
        class="w-20 h-20 bg-linear-to-br from-brand-blue/10 to-brand-purple/10 rounded-full flex items-center justify-center mb-6"
      >
        <svg
          class="w-10 h-10 text-brand-blue"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      <button
        routerLink="/stt"
        [queryParams]="{ autostart: 'PRO_PLUS' }"
