    ...
    <div class="min-h-screen bg-primary-50 dark:bg-primary-950">
      ...
      >
        ...
        <div class="mb-4 flex items-start justify-between">
          ...
          <button
            routerLink="/tts"
            class="p-2 -mt-2 rounded-xl text-primary-400 hover:text-primary-600 dark:hover:text-primary-200 hover:bg-white dark:hover:bg-primary-900 border border-transparent hover:border-primary-200 dark:hover:border-primary-700 transition-all group shadow-sm hover:shadow-md shrink-0"
              ...
              <tbody>
                @if (!loading()) {
                  @for (item of history(); track item) {
                    ...
                    >
                      <td
                        class="px-4 py-2.5 md:py-5 border-b border-r border-primary-100 dark:border-primary-800 whitespace-nowrap"
                      >
                        <span class="text-xs sm:text-sm font-bold text-brand-blue">{{
                          item.planName === 'FREE' ? 'Basic' : item.planName.replace('_', ' ')
                        }}</span>
                      </td>
                      <td
                        class="px-4 py-2.5 md:py-5 border-b border-r border-primary-100 dark:border-primary-800 whitespace-nowrap"
                      >
                        <span
                          ...
                          >{{ item.amount | currency: item.currency : 'symbol' : '1.2-2' }}</span
                        >
                      </td>
                      <td
                        class="px-4 py-2.5 md:py-5 border-b border-r border-primary-100 dark:border-primary-800 whitespace-nowrap"
                      >
                        <span
                          class="px-2 py-1 rounded text-[10px] sm:text-xs font-bold capitalize"
                          [ngClass]="{
                            'bg-green-500/10 text-green-600':
                              item.status === 'SUCCESS' || item.status === 'CAPTURED',
                      ...
                      <td
                        class="px-4 py-2.5 md:py-5 border-b border-primary-100 dark:border-primary-800 whitespace-nowrap"
                      >
