...
@Component({
  ...
  template: `
    <header
      class="sticky top-0 z-50 w-full bg-white dark:bg-primary-900 border-b border-primary-200 dark:border-primary-800"
    >
        ...
        <div class="flex items-center justify-between h-16 gap-4">
          ...
          <div class="flex items-center gap-1.5 sm:gap-2 min-w-0">
            ...
            >
              <div class="w-5 h-4 flex flex-col justify-between relative overflow-hidden">
                <span
                  class="w-full h-0.5 rounded-full transition-smooth origin-center"
                  [ngClass]="
                    showMobileMenu()
            ...
            <a routerLink="/" class="flex items-center gap-2 sm:gap-3 group shrink-0">
              <div
                class="w-8 h-8 rounded-lg bg-gradient-to-br from-brand-blue to-brand-purple flex items-center justify-center shadow-md group-hover:shadow-lg group-hover:scale-105 transition-all"
              >
                <svg
              ...
              <span
                class="text-base sm:text-lg font-semibold text-primary-900 dark:text-white tracking-tight"
                >SpeakIT</span
              >
            </a>
          </div>
          ...
          <div class="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            @if (authService.currentUser(); as user) {
              ...
              @if (authService.currentPlanType() !== 'ENTERPRISE') {
                <a
                  [routerLink]="getNextPlan() === 'ENTERPRISE' ? '/contact' : '/tts'"
                  [queryParams]="
                    getNextPlan() === 'ENTERPRISE'
                      ? { topic: 'enterprise' }
                      : { autostart: getNextPlan() }
                  "
                ...
                >
                  <svg
                    class="w-3.5 h-3.5 group-hover/upg:scale-110 transition-transform"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              <div class="flex items-center gap-2 sm:gap-3 relative">
                ...
                <div
                  ...
                  [ngClass]="{
                    'bg-accent-50 dark:bg-accent-500/10 border-accent-200 dark:border-accent-500/30':
                      authService.currentPlanType() !== 'FREE',
                  }"
                >
                  <div class="relative">
                    <div
                      class="w-6 h-6 rounded-full bg-gradient-to-br from-brand-blue to-brand-purple flex items-center justify-center shrink-0"
                    >
                    ...
                    <span
                      class="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white dark:border-primary-900"
                      [ngClass]="
                        authService.currentPlanType() !== 'FREE'
                          ? 'bg-emerald-500'
                          : 'bg-primary-400'
                  ...
                  <span
                    class="text-xs font-semibold text-primary-900 dark:text-white truncate max-w-[120px]"
                    >{{ user }}</span
                  >
                  <svg
                    class="w-4 h-4 text-primary-400 group-hover:text-primary-600 transition-colors"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                @if (showUserMenu()) {
                  ...
                  >
                    ...
                    <div
                      class="px-3.5 py-2.5 mx-2.5 mt-1.5 bg-gradient-to-r from-brand-blue/5 to-brand-purple/5 dark:from-brand-blue/10 dark:to-brand-purple/10 border border-brand-blue/10 dark:border-brand-purple/20 rounded-xl mb-2 flex items-center justify-between"
                    >
                      <div class="flex flex-col min-w-0">
                        <span
                          class="text-[9px] font-bold text-primary-400 tracking-widest uppercase"
                          >Plan</span
                        >
                        <div class="flex items-center gap-1.5 mt-0.5">
                          ...
                          @if (authService.currentPlanType() !== 'FREE') {
                            <span class="flex h-1.5 w-1.5 relative">
                              <span
                                class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-450 opacity-75"
                              ></span>
                              <span
                                class="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-550"
                              ></span>
                            </span>
                          }
                        </div>
      ...
      @if (showMobileMenu()) {
        ...
        >
          <nav class="flex flex-col gap-0.5">
            @for (link of navLinks; track link; let i = $index) {
              <a
                [routerLink]="link.path"
                [fragment]="link.fragment"
                (click)="showMobileMenu.set(false)"
              ...
              >
                @switch (link.label) {
                  @case ('Home') {
                    <svg
                      class="w-5 h-5 text-primary-400 dark:text-primary-500 shrink-0"
                      width="20"
                      height="20"
                      fill="none"
                      stroke="currentColor"
            ...
            <div
              class="sm:hidden border-t border-primary-100 dark:border-primary-800/60 mt-1.5 pt-1.5"
            >
              <button
                (click)="themeService.toggleTheme(); showMobileMenu.set(false)"
...
export class NavbarComponent implements OnDestroy {
  ...
  @ViewChild('hamburgerButton', { read: ElementRef })
  hamburgerButton!: ElementRef<HTMLButtonElement>;
